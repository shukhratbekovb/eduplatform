...
async def _process_lesson_attendance(lesson_id: str) -> dict:  # type: ignore[type-arg]
    ...
    async with async_session_factory() as session:
        # Get lesson info
        lesson = (
            await session.execute(select(LessonModel).where(LessonModel.id == UUID(lesson_id)))
        ).scalar_one_or_none()
        if not lesson:
        ...
        absent_records = (
            (
                await session.execute(
                    select(AttendanceRecordModel.student_id).where(
        ...
        for student_id in absent_records:
            ...
            student = (
                await session.execute(
                    select(StudentModel.full_name, StudentModel.phone, StudentModel.parent_phone).where(
            ...
            if streak >= 3 and not await _task_exists(session, student_id, "absence_streak"):
                for mup_id in mup_ids:
                    parent_info = (
                        f"Тел. родителя: {student.parent_phone}" if student.parent_phone else "Тел. родителя не указан"
                    )
                    session.add(
                        MupTaskModel(
                            id=uuid4(),
