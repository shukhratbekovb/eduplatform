...
async def _student_has_docs(db, student_id: UUID) -> bool:
    ...
    r = (
        await db.execute(text("SELECT count(*) FROM student_documents WHERE student_id = :sid"), {"sid": student_id})
    ).scalar()
    return (r or 0) > 0


def _contract_out(
    m: ContractModel, direction=None, login=None, password=None, has_docs=False, documents=None
) -> ContractOut:  # type: ignore[no-untyped-def]
    return ContractOut(
        id=m.id,
        contractNumber=getattr(m, "contract_number", None),
        leadId=m.lead_id,
...
async def upload_document(
    ...
    return DocumentOut(
        id=doc_id,
        type=doc_type,
        typeLabel=DOCUMENT_TYPES[doc_type],
...
async def list_student_documents(
    ...
    return [
        DocumentOut(
            id=r[0],
            type=r[1],
            typeLabel=DOCUMENT_TYPES.get(r[1], r[1]),
            fileUrl=r[2],
            fileName=r[3],
            uploadedAt=r[4].isoformat() if r[4] else None,
        )
        for r in rows
    ]
...
async def create_contract(
    ...
    if student_id:
        ...
        if existing_user:
            ...
            s = (
                await db.execute(select(StudentModel).where(StudentModel.user_id == existing_user.id))
            ).scalar_one_or_none()
            if s:
                student_id = s.id
        else:
...
async def list_contracts(
    ...
    if search:
        q = q.where(
            ContractModel.full_name.ilike(f"%{search}%")
            | ContractModel.phone.ilike(f"%{search}%")
    ...
    rows = (
        (await db.execute(q.order_by(ContractModel.created_at.desc()).offset((page - 1) * limit).limit(limit)))
        .scalars()
        .all()
    )
