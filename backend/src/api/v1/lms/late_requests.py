...
async def list_requests(
    ...
    rows = (await db.execute(
        q.order_by(LateEntryRequestModel.created_at.desc())
...
async def create_request(body: CreateLateRequestIn, current_user: CurrentUser, db: DbSession) -> LateRequestOut:
    ...
    return LateRequestOut(
        ...
        reason=r.reason, is_approved=r.is_approved,
        created_at=r.created_at.isoformat() if r.created_at else None,
    )
...
async def review_request(
    request_id: UUID, body: ReviewLateRequestIn, _: StaffGuard, current_user: CurrentUser, db: DbSession,
) -> LateRequestOut:
    r = (await db.execute(select(LateEntryRequestModel).where(LateEntryRequestModel.id == request_id))).scalar_one_or_none()
    if r is None:
