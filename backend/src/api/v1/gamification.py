...
async def list_shop_items(current_user: CurrentUser, db: DbSession) -> list[ShopItemOut]:
    ...
    """
    rows = (await db.execute(
        select(ShopItemModel).where(ShopItemModel.is_active == True)  # noqa: E712
        .order_by(ShopItemModel.cost_stars)
    )).scalars().all()
    return [ShopItemOut(
        id=m.id, name=m.name, description=m.description, icon=m.icon,
        category=m.category, cost_stars=m.cost_stars, cost_crystals=m.cost_crystals,
        stock=m.stock, is_active=m.is_active,
    ) for m in rows]
...
async def purchase_item(
    body: PurchaseRequest, current_user: CurrentUser, db: DbSession,
) -> dict:  # type: ignore[type-arg]
    ...
    now = datetime.now(timezone.utc)
    db.add(StudentPurchaseModel(
        id=uuid4(), student_id=student.id, item_id=item.id, purchased_at=now,
    ))
    db.add(StudentActivityEventModel(
...
async def achievement_catalog(current_user: CurrentUser, db: DbSession) -> list[AchievementCatalogOut]:
    ...
    if student:
        rows = (await db.execute(
            select(StudentAchievementModel)
    ...
    return [
        AchievementCatalogOut(
            ...
            reward_stars=a.reward_stars, reward_crystals=a.reward_crystals,
