...
import { Plus, Copy, Check, Radio, Link2, Globe, FileSpreadsheet, Pencil, Lock, Trash2 } from 'lucide-react'
...
export default function SourcesPage() {
  ...
  const [form, setForm] = useState<CreateSourceDto>({ name: '', type: 'landing' })
  ...
  const copyToClipboard = (text: string) => {
    ...
    setCopied(text)
    toast.success(t('sources.copied'))
    setTimeout(() => setCopied(null), 2000)
  }

  const handleCreate = () => {
    if (!form.name.trim()) { toast.error(t('sources.errorName')); return }
    if (!form.funnelId) { toast.error(t('sources.errorFunnel')); return }
    createSource.mutate(form, {
      onSuccess: () => {
        setShowCreate(false)
        setForm({ name: '', type: 'landing' })
      },
    })
  }
  ...
  const handleDelete = (id: string) => {
    deleteSource.mutate(id, {
      onSuccess: () => { setDeleteId(null); toast.success(t('sources.deleted')) },
      onError: () => { toast.error(t('sources.deleteError')) },
    })
  }
  ...
  const getEndpointUrl = (source: LeadSource) => {
    if (source.type === 'api') return `${API_URL}/public/api/${source.apiKey}/leads`
    if (source.type === 'landing') return `${WEBSITE_URL}/form/${source.apiKey}`
    return null
  }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{t('sources.subtitle')}</p>
        </div>
      ...
      {showCreate && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 space-y-4">
          <h3 className="text-sm font-semibold text-gray-700">{t('sources.newSource')}</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('sources.name')}</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('sources.type')}</label>
              <select
                className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500"
                value={form.type}
                onChange={(e) => setForm({ ...form, type: e.target.value as 'api' | 'landing' })}
              >
                <option value="landing">{t('sources.typeLanding')}</option>
                <option value="api">{t('sources.typeApi')}</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">{t('sources.funnel')}</label>
              <select
                className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500"
                value={form.funnelId || ''}
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={() => { setShowCreate(false); setForm({ name: '', type: 'landing' }) }}>
              {t('common.cancel')}
            </Button>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          action={{ label: t('sources.addSource'), onClick: () => setShowCreate(true) }}
        />
      ) : (
        <div className="space-y-3">
          {sources.map((source) => {
            ...
            const isSystem = !!source.isSystemSource

            return (
              <div key={source.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isSystem ? 'bg-gray-200' : 'bg-gray-100'}`}>
                      {isSystem ? (
                    ...
                    <div>
                      ...
                      {isSystem ? (
                        <span className="text-xs text-gray-400">{t('sources.systemHint')}</span>
                      ) : funnelName ? (
                        <span className="text-xs text-gray-400">{t('sources.funnel')}: {funnelName}</span>
                      ) : null}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {isSystem && (
                      <Badge variant="default">{t('sources.systemBadge')}</Badge>
                    )}
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <span className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider">
                          {source.type === 'api' ? t('sources.apiEndpoint') : t('sources.landingUrl')}
                        </span>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('sources.deleteTitle')}</h3>
            <p className="text-sm text-gray-500 mb-4">{t('sources.deleteDesc')}</p>
            <div className="flex justify-end gap-2">
              <Button size="sm" variant="ghost" onClick={() => setDeleteId(null)}>{t('common.cancel')}</Button>
              <Button size="sm" variant="danger" onClick={() => handleDelete(deleteId)} loading={deleteSource.isPending}>{t('common.delete')}</Button>
            </div>
          </div>
        </div>
      )}
