...
const COLUMN_STYLE: Record<TaskStatus, { title: string; header: string; dot: string }> = {
  pending:     { title: 'Ожидает', header: 'bg-gray-50 border-gray-200',   dot: 'bg-gray-400' },
  in_progress: { title: 'В работе', header: 'bg-info-50 border-info-200', dot: 'bg-info-500' },
  done:        { title: 'Готово',   header: 'bg-success-50 border-success-200', dot: 'bg-success-500' },
  overdue:     { title: 'Просрочено', header: 'bg-danger-50 border-danger-200', dot: 'bg-danger-500' },
}
      ...
      >
        ...
        {tasks.length === 0 && (
          <div className="flex items-center justify-center h-24 text-xs text-gray-300">
            {isReadOnly ? 'Нет просроченных задач' : 'Нет задач'}
          </div>
        )}
      </div>
    </div>
